...
const UserController = {
    create: async(req, res) => {
        try{
            ...
            //Xử lý
            var exists = await User.findOne({ username: formData.username });
            if(exists == null) {
                //Xử lý upload file
                if(req.file){
                ...
                const newData = await new User({
                    ...
                    "address.wardId": formData.address.wardId ? parseInt(formData.address.wardId, 10) : 0,
                    "address.districtId": formData.address.districtId ? parseInt(formData.address.districtId, 10) : 0,
                    "address.provinceId": formData.address.provinceId ? parseInt(formData.address.provinceId, 10) : 0,
                ...
                return res.status(200).json({ success: false, error: 'Tài khoản đã tồn tại' });
            }
        }
        catch(err){
            ...
            await User.updateOne(
                { _id: formData._id }, 
                {
                    $set: { 
                        ...
                        "address.wardId": parseInt(formData.address.wardId, 10),
                        "address.districtId": parseInt(formData.address.districtId, 10),
                        "address.provinceId": parseInt(formData.address.provinceId, 10),
                        img: formData.img,
                        imageFile: null,
                        accessId: formData.accessId,
